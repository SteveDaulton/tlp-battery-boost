...
- User manual via MkDocs, published at https://stevedaulton.github.io/tlp-battery-boost/

### Changed

- Minor updates to README.
- Refactored theming and font-size handling to improve maintainability and
  type checking.

### Fixed

- Ensure logging shows correct debug level when `DEBUG` is enabled.
- Minor docstring corrections and formatting improvements.
...
- Treat attempting `tlp fullcharge` while on battery power as non-fatal
  (Fixes **#2**). The user may now attach AC and retry without the
  application exiting.
...
First stable release of **Battery Boost**, providing a lightweight graphical
interface for managing TLP battery-charging modes.

### Added

- **Tkinter GUI application** for toggling between TLP’s normal battery-optimised
  mode and full-charge mode (`tlp fullcharge`).
- **Battery status display** using `tlp-stat -b`, showing charge levels and
...
- **Documentation:** Comprehensive README including installation, usage, screenshots,
